...
-- Allow owners to delete/update their objects
CREATE POLICY modify_objects_owner ON public.objects
  FOR UPDATE, DELETE
  USING (owner_uuid = auth.uid()::uuid)
  WITH CHECK (owner_uuid = auth.uid()::uuid);
...
CREATE POLICY insert_signatures_auth ON public.signatures
  FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.objects o WHERE o.cid = new.cid AND o.owner_uuid = auth.uid()::uuid));
