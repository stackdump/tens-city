...
GO := go
GOFLAGS := -v
BINARIES := seal keygen webserver
BIN_DIR := bin
CMD_DIR := cmd
STORE_DIR := data
...
# Phony targets
.PHONY: all build test clean fmt vet lint help install run-webserver run-seal $(BINARIES)
...
# Build all binaries
build: $(BINARIES) ## Build all binaries (seal, keygen, webserver)

# Build individual binaries
seal: ## Build seal binary
	$(GO) build $(GOFLAGS) -ldflags "$(LDFLAGS)" -o $@ ./$(CMD_DIR)/$@

keygen: ## Build keygen binary
	$(GO) build $(GOFLAGS) -ldflags "$(LDFLAGS)" -o $@ ./$(CMD_DIR)/$@
...
install: ## Install binaries to $GOPATH/bin
	$(GO) install $(GOFLAGS) -ldflags "$(LDFLAGS)" ./$(CMD_DIR)/seal
	$(GO) install $(GOFLAGS) -ldflags "$(LDFLAGS)" ./$(CMD_DIR)/keygen
...
run-seal: seal ## Build and run seal with example file
	./seal -in examples/petrinet.jsonld -store $(STORE_DIR)
...
quick: ## Quick build without optimization flags
	$(GO) build -o seal ./$(CMD_DIR)/seal
	$(GO) build -o keygen ./$(CMD_DIR)/keygen
